...
SEARCH_URL = "https://jobs.canadalife.com/search"
BASE_URL = "https://jobs.canadalife.com"
PAGE_SIZE = 10
...
def scrape_canada_life() -> list[dict]:
    jobs = _try_json_api()
    if not jobs:
...
def _try_json_api() -> list[dict]:
    ...
    try:
        ...
        logger.warning(f"Canada Life JSON API failed: {e}")
        return []
    return jobs
...
def _try_html_scrape() -> list[dict]:
    ...
    try:
        ...
        logger.warning(f"Canada Life HTML scrape failed: {e}")
    return jobs
